...
test("optional exported snapshot passes the real worker integrity, schema, and query pipeline", {
  ...
  const databaseBytes = new Uint8Array(await readFile(join(snapshotDirectory, rawManifest.database.path)));
  ...
  const workerLocation = new URL("https://catalog.example/catalog-worker.js");
  ...
  globalThis.fetch = async (input) => {
    ...
    if (url.pathname === "/catalog-manifest.json") {
      ...
      return new Response(JSON.stringify(selectedManifest), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (url.pathname === `/${rawManifest.database.path}`) {
      return new Response(databaseBytes, { status: 200, headers: { "content-type": "application/vnd.sqlite3" } });
    }
    if (url.pathname === "/vendor/sqlite/sqlite3.wasm") {
  ...
  try {
    ...
    const initialized = await request("init", { manifestUrl: "https://catalog.example/catalog-manifest.json" });
