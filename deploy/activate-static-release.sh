...
# The exporter guarantees exactly one public database and two precompressed
# representations. Recheck their exact names plus the decoded database byte
# count and content-addressed SHA-256 before changing the live pointer.
set -- "$release_dir"/data/catalog-*.sqlite3
[ "$#" -eq 1 ] && [ -f "$1" ] || fail "release must contain exactly one data/catalog-<sha256>.sqlite3 file"
database_path=$1
database_name=${database_path##*/}
brotli_path="$database_path.br"
gzip_path="$database_path.gz"
[ -s "$brotli_path" ] || fail "release is missing the non-empty Brotli database representation: $brotli_path"
[ -s "$gzip_path" ] || fail "release is missing the non-empty gzip database representation: $gzip_path"
set -- "$release_dir"/data/catalog-*.sqlite3.br
[ "$#" -eq 1 ] && [ "$1" = "$brotli_path" ] || fail "release must contain exactly one matching .sqlite3.br representation"
set -- "$release_dir"/data/catalog-*.sqlite3.gz
[ "$#" -eq 1 ] && [ "$1" = "$gzip_path" ] || fail "release must contain exactly one matching .sqlite3.gz representation"
